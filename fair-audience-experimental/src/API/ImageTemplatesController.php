...
class ImageTemplatesController extends WP_REST_Controller {
	...
	public function create_item( $request ) {
		...
		if ( ! $attachment || 'attachment' !== $attachment->post_type ) {
			return new WP_Error(
				'invalid_attachment',
				__( 'Invalid attachment ID.', 'fair-audience-experimental' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( 'image/svg+xml' !== $mime_type ) {
			return new WP_Error(
				'invalid_mime_type',
				__( 'Only SVG files can be used as image templates.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( false === $svg_content ) {
			return new WP_Error(
				'file_read_error',
				__( 'Could not read SVG file.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
	...
	public function delete_item( $request ) {
		...
		if ( ! $attachment || 'attachment' !== $attachment->post_type ) {
			return new WP_Error(
				'not_found',
				__( 'Template not found.', 'fair-audience-experimental' ),
				array( 'status' => 404 )
			);
		}
	...
	public function render_item( $request ) {
		...
		if ( '1' !== $meta ) {
			return new WP_Error(
				'not_template',
				__( 'Attachment is not registered as a template.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
