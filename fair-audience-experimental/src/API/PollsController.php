...
use FairAudienceExperimental\Database\PollOptionRepository;
...
use FairAudienceExperimental\Models\Poll;
...
class PollsController extends WP_REST_Controller {
	...
	public function get_item( $request ) {
		...
		if ( ! $poll ) {
			return new WP_Error(
				'poll_not_found',
				__( 'Poll not found.', 'fair-audience-experimental' ),
				array( 'status' => 404 )
			);
		}
	...
	public function create_item( $request ) {
		...
		if ( ! $event || ! \FairEvents\Database\EventRepository::is_event( $event ) ) {
			return new WP_Error(
				'invalid_event',
				__( 'Invalid event ID.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $poll->save() ) {
			return new WP_Error(
				'poll_create_failed',
				__( 'Failed to create poll.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		if ( ! empty( $options ) ) {
			if ( ! $this->option_repository->bulk_create( $poll->id, $options ) ) {
				return new WP_Error(
					...
					__( 'Failed to create poll options.', 'fair-audience-experimental' ),
					array( 'status' => 500 )
				);
			}
		}
		...
		return rest_ensure_response(
			array(
				'poll_id' => $poll->id,
				'message' => __( 'Poll created successfully.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function update_item( $request ) {
		...
		if ( ! $poll->save() ) {
			return new WP_Error(
				'poll_update_failed',
				__( 'Failed to update poll.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
			...
			if ( ! empty( $options ) ) {
				if ( ! $this->option_repository->bulk_create( $id, $options ) ) {
					return new WP_Error(
						...
						__( 'Failed to update poll options.', 'fair-audience-experimental' ),
						array( 'status' => 500 )
					);
				}
			}
		...
		return rest_ensure_response(
			array(
				'message' => __( 'Poll updated successfully.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function delete_item( $request ) {
		...
		if ( ! $this->poll_repository->delete( $id ) ) {
			return new WP_Error(
				'poll_delete_failed',
				__( 'Failed to delete poll.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				'deleted' => true,
				'message' => __( 'Poll deleted successfully.', 'fair-audience-experimental' ),
			)
		);
	}
