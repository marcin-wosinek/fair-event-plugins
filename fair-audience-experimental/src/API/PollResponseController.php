...
class PollResponseController extends WP_REST_Controller {
	...
	public function submit( $request ) {
		...
		if ( $attempts >= 5 ) {
			return new WP_Error(
				'rate_limit_exceeded',
				__( 'Too many submission attempts. Please try again later.', 'fair-audience-experimental' ),
				array( 'status' => 429 )
			);
		}
		...
		if ( ! $access_key ) {
			return new WP_Error(
				'invalid_access_key',
				__( 'Invalid or expired access key.', 'fair-audience-experimental' ),
				array( 'status' => 403 )
			);
		}
		...
		if ( 'responded' === $access_key->status ) {
			return new WP_Error(
				'already_responded',
				__( 'You have already responded to this poll.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( 'active' !== $poll->status ) {
			return new WP_Error(
				'poll_not_active',
				__( 'This poll is no longer accepting responses.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( empty( $selected_option_ids ) ) {
			return new WP_Error(
				'no_options_selected',
				__( 'Please select at least one option.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		foreach ( $selected_option_ids as $option_id ) {
			if ( ! in_array( $option_id, $valid_option_ids, true ) ) {
				return new WP_Error(
					'invalid_option',
					__( 'Invalid option selected.', 'fair-audience-experimental' ),
					array( 'status' => 400 )
				);
			}
		}
		...
		if ( ! $this->response_repository->save_responses( $poll->id, $access_key->participant_id, $selected_option_ids ) ) {
			return new WP_Error(
				'save_failed',
				__( 'Failed to save your response. Please try again.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				'success' => true,
				'message' => __( 'Thank you for your response!', 'fair-audience-experimental' ),
			)
		);
	}
}
