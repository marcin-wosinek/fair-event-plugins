...
use FairAudienceExperimental\Database\TimelineRepository;
...
class TimelineController extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		foreach ( $submissions as $row ) {
			...
			$form_name  = $row['title'] ? $row['title'] : __( 'Fair Form', 'fair-audience-experimental' );
			...
			if ( $page_title ) {
				$summary = sprintf(
					...
					__( '%1$s submitted "%2$s" on "%3$s"', 'fair-audience-experimental' ),
					$name,
					$form_name,
					$page_title
				);
				...
				$summary = sprintf(
					...
					__( '%1$s submitted "%2$s"', 'fair-audience-experimental' ),
					$name,
					$form_name
				);
			}
		...
		if ( $include_payments ) {
			...
			foreach ( $payment_rows as $row ) {
				...
				$name = trim( $row['participant_name'] . ' ' . $row['participant_surname'] );
				if ( '' === $name ) {
					$name = __( 'Unknown', 'fair-audience-experimental' );
				}
