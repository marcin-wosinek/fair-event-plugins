...
class FeesController extends WP_REST_Controller {
	...
	public function get_item( $request ) {
		...
		if ( ! $data ) {
			return new WP_Error(
				'fee_not_found',
				__( 'Fee not found.', 'fair-audience-experimental' ),
				array( 'status' => 404 )
			);
		}
	...
	public function create_item( $request ) {
		...
		if ( empty( $name ) ) {
			return new WP_Error(
				'missing_name',
				__( 'Fee name is required.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( empty( $group_id ) ) {
			return new WP_Error(
				'missing_group',
				__( 'Group is required.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! is_numeric( $amount ) || $amount <= 0 ) {
			return new WP_Error(
				'invalid_amount',
				__( 'Amount must be a positive number.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $fee->save() ) {
			return new WP_Error(
				'creation_failed',
				__( 'Failed to create fee.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				...
				'message'          => __( 'Fee created successfully.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function update_item( $request ) {
		...
		if ( ! $fee->save() ) {
			return new WP_Error(
				'update_failed',
				__( 'Failed to update fee.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				'id'      => $fee->id,
				'message' => __( 'Fee updated successfully.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function delete_item( $request ) {
		...
		if ( ! $fee->delete() ) {
			return new WP_Error(
				'deletion_failed',
				__( 'Failed to delete fee.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				'message' => __( 'Fee deleted successfully.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function add_payment( $request ) {
		...
		if ( $participant_id <= 0 ) {
			return new WP_Error(
				'missing_participant',
				__( 'Participant is required.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! is_numeric( $amount ) || $amount < 0 ) {
			return new WP_Error(
				'invalid_amount',
				__( 'Amount must be a non-negative number.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $is_member ) {
			return new WP_Error(
				'not_in_group',
				__( 'Participant is not a member of this fee\'s group.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( $existing ) {
			return new WP_Error(
				'payment_exists',
				__( 'This participant already has a payment for this fee.', 'fair-audience-experimental' ),
				array( 'status' => 409 )
			);
		}

		$payment = new \FairAudienceExperimental\Models\FeePayment();
		$payment->populate(
			array(
				'fee_id'         => (int) $fee_id,
		...
		if ( ! $payment->save() ) {
			return new WP_Error(
				'creation_failed',
				__( 'Failed to add participant to fee.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		$this->audit_log_repository->log_action(
			...
			null,
			$amount,
			__( 'Participant added manually.', 'fair-audience-experimental' )
		);

		return rest_ensure_response(
			array(
				'id'      => $payment->id,
				'message' => __( 'Participant added to fee.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function adjust_payment_amount( $request ) {
		...
		if ( empty( $comment ) ) {
			return new WP_Error(
				'missing_comment',
				__( 'A reason for the adjustment is required.', 'fair-audience-experimental' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $payment->save() ) {
			return new WP_Error(
				'update_failed',
				__( 'Failed to update payment amount.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				'message' => __( 'Payment amount adjusted successfully.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function mark_payment_paid( $request ) {
		...
		if ( ! $payment->save() ) {
			return new WP_Error(
				'update_failed',
				__( 'Failed to mark payment as paid.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				'message' => __( 'Payment marked as paid.', 'fair-audience-experimental' ),
			)
		);
	}
	...
	public function cancel_payment( $request ) {
		...
		if ( ! $payment->save() ) {
			return new WP_Error(
				'update_failed',
				__( 'Failed to cancel payment.', 'fair-audience-experimental' ),
				array( 'status' => 500 )
			);
		}
		...
		return rest_ensure_response(
			array(
				'message' => __( 'Payment canceled.', 'fair-audience-experimental' ),
			)
		);
	}
	 ...
	 *
	 * @param WP_REST_Request $request Request object.
	 * @return \FairAudienceExperimental\Models\FeePayment|WP_Error Payment or error.
	 */
	private function get_validated_payment( $request ) {
		...
		if ( ! $payment || $payment->fee_id !== (int) $fee_id ) {
			return new WP_Error(
				'payment_not_found',
				__( 'Payment not found.', 'fair-audience-experimental' ),
				array( 'status' => 404 )
			);
		}
