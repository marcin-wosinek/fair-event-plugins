...
class InstagramPostingService {
	...
	public function is_configured() {
		...
		if ( empty( $user_id ) ) {
			return new WP_Error(
				...
				__( 'Instagram user ID is not configured. Please connect your Instagram account in Settings.', 'fair-audience-experimental' )
			);
		}
		...
		if ( empty( $access_token ) ) {
			return new WP_Error(
				'instagram_not_configured',
				__( 'Instagram access token is not configured. Please connect your Instagram account in Settings.', 'fair-audience-experimental' )
			);
		}
	...
	private function create_media_container( $user_id, $access_token, $image_url, $caption ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					...
					__( 'Failed to connect to Instagram API: %s', 'fair-audience-experimental' ),
		...
		if ( isset( $data['error'] ) ) {
			...
			$error_message = $data['error']['error_user_msg']
				?? $data['error']['message']
				?? __( 'Unknown error', 'fair-audience-experimental' );
			return new WP_Error(
				'instagram_error',
				sprintf(
					/* translators: %s: error message */
					__( 'Instagram API error: %s', 'fair-audience-experimental' ),
					$error_message
				)
			);
		}
		...
		if ( empty( $data['id'] ) ) {
			...
			return new WP_Error(
				'invalid_response',
				__( 'Invalid response from Instagram API: no container ID returned.', 'fair-audience-experimental' )
			);
		}
	...
	private function wait_for_container( $access_token, $container_id ) {
		...
		for ( $i = 0; $i < $max_attempts; $i++ ) {
			...
			if ( ! empty( $data['status_code'] ) ) {
				...
				if ( 'ERROR' === $data['status_code'] ) {
					...
					return new WP_Error(
						'container_error',
						__( 'Instagram failed to process the image.', 'fair-audience-experimental' )
					);
				}
			}
		}
		...
		return new WP_Error(
			'container_timeout',
			__( 'Instagram took too long to process the image. Please try again.', 'fair-audience-experimental' )
		);
	}
	...
	private function publish_container( $user_id, $access_token, $container_id ) {
		...
		if ( is_wp_error( $response ) ) {
			...
			return new WP_Error(
				...
				sprintf(
					/* translators: %s: error message */
					__( 'Failed to publish to Instagram: %s', 'fair-audience-experimental' ),
					$response->get_error_message()
				)
			);
		}
		...
		if ( isset( $data['error'] ) ) {
			...
			$error_message = $data['error']['message'] ?? __( 'Unknown error', 'fair-audience-experimental' );
			return new WP_Error(
				'instagram_error',
				sprintf(
					/* translators: %s: error message */
					__( 'Failed to publish: %s', 'fair-audience-experimental' ),
					$error_message
				)
			);
		}
		...
		if ( empty( $data['id'] ) ) {
			...
			return new WP_Error(
				'invalid_response',
				__( 'Invalid response from Instagram API: no media ID returned.', 'fair-audience-experimental' )
			);
		}
