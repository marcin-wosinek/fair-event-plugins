...
const LABEL_OPTIONS = [
	{
		key: 'signed_up',
		label: __( 'Registered', 'fair-audience-experimental' ),
	},
	{
		key: 'interested',
...
export default function EventMailings( {
	...
	const selectedLabels = useMemo(
		() =>
			LABEL_OPTIONS.filter( ( o ) => form.labels[ o.key ] ).map(
				( o ) => o.key
			),
		[ form.labels ]
	);
							...
							{ groups.length > 0 && (
								<div style={ { marginTop: '8px' } }>
									<FormTokenField
										...
										value={ form.groupIds.map(
											groupNameById
										) }
										suggestions={ groups.map(
											( g ) => g.name
										) }
										onChange={ ( names ) =>
											updateForm( {
