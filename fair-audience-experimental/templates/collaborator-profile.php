...
if ( isset( $_SERVER['REQUEST_METHOD'] ) && 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['collaborator_profile_submit'] ) ) {
	...
	if ( empty( $form_values['name'] ) ) {
		$result['type']    = 'error';
		$result['message'] = __( 'Name is required.', 'fair-audience-experimental' );
	} elseif ( ! empty( $form_values['email'] ) && ! is_email( $form_values['email'] ) ) {
		$result['type']    = 'error';
		$result['message'] = __( 'Please enter a valid email address.', 'fair-audience-experimental' );
	} elseif ( ! empty( $form_values['email'] ) && $participant_repository->get_by_email( $form_values['email'] ) ) {
		$result['type']    = 'error';
		$result['message'] = __( 'A profile with this email already exists.', 'fair-audience-experimental' );
	} else {
		...
		if ( ! $participant->save() ) {
			$result['type']    = 'error';
			$result['message'] = __( 'Failed to register profile. Please try again.', 'fair-audience-experimental' );
		} elseif ( $has_email ) {
	...
	<div class="fair-audience-profile-box">
		<?php if ( $result['success'] ) : ?>
			...
			<h1 class="fair-audience-profile-title">
				<?php echo esc_html__( 'Profile Registered', 'fair-audience-experimental' ); ?>
			</h1>
			<?php if ( $result['confirmation_sent'] ) : ?>
				<p class="fair-audience-profile-subtitle">
					<?php echo esc_html__( 'Please check your email to confirm your subscription.', 'fair-audience-experimental' ); ?>
				</p>
			<?php else : ?>
				<p class="fair-audience-profile-subtitle">
					<?php echo esc_html__( 'Thank you for registering your profile.', 'fair-audience-experimental' ); ?>
				</p>
			<?php endif; ?>
		...
		<?php else : ?>
			<h1 class="fair-audience-profile-title">
				<?php echo esc_html__( 'Collaborator Profile', 'fair-audience-experimental' ); ?>
			</h1>
			<p class="fair-audience-profile-subtitle">
				<?php echo esc_html__( 'Register your profile information.', 'fair-audience-experimental' ); ?>
			</p>
			...
			<form method="post">
				...
				<div class="fair-audience-profile-field">
					<label class="fair-audience-profile-label" for="fair-audience-name">
						<?php echo esc_html__( 'Name', 'fair-audience-experimental' ); ?>
				...
				<div class="fair-audience-profile-field">
					<label class="fair-audience-profile-label" for="fair-audience-surname">
						<?php echo esc_html__( 'Surname', 'fair-audience-experimental' ); ?>
					</label>
					<input
						type="text"
				...
				<div class="fair-audience-profile-field">
					<label class="fair-audience-profile-label" for="fair-audience-email">
						<?php echo esc_html__( 'Email', 'fair-audience-experimental' ); ?>
					</label>
					<input
						type="email"
				...
				<div class="fair-audience-profile-field">
					<label class="fair-audience-profile-label" for="fair-audience-instagram">
						<?php echo esc_html__( 'Instagram', 'fair-audience-experimental' ); ?>
					</label>
					<input
						type="text"
				...
				<div class="fair-audience-profile-checkbox">
					...
					<label class="fair-audience-profile-checkbox-label" for="fair-audience-marketing">
						<?php echo esc_html__( 'Receive marketing emails', 'fair-audience-experimental' ); ?>
					</label>
				</div>

				<button type="submit" class="fair-audience-profile-submit">
					<?php echo esc_html__( 'Register Profile', 'fair-audience-experimental' ); ?>
				</button>
			</form>
		<?php endif; ?>
	</div>
