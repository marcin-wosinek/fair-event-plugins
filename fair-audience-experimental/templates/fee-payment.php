...
if ( false === $fee_payment_id ) {
	$result['message'] = __( 'Invalid or expired link. Please use the link from your email.', 'fair-audience-experimental' );
} else {
	// Get fee payment.
	$fee_payment = $payment_repository->get_by_id( $fee_payment_id );

	if ( ! $fee_payment ) {
		$result['message'] = __( 'Payment not found.', 'fair-audience-experimental' );
	} else {
		$fee         = $fee_repository->get_by_id( $fee_payment->fee_id );
		$participant = $participant_repository->get_by_id( $fee_payment->participant_id );

		if ( ! $fee || ! $participant ) {
			$result['message'] = __( 'Payment details not found.', 'fair-audience-experimental' );
		} else {
			$result['success']     = true;
			...
			if ( $fee_payment->transaction_id && 'pending' === $fee_payment->status ) {
				...
				if ( $transaction && 'paid' === $transaction->status ) {
					...
					$result['type']    = 'warning';
					$result['message'] = __( 'Your payment was not completed. You can try again.', 'fair-audience-experimental' );
					...
					if ( $age_minutes > 20 ) {
						$result['type']    = 'warning';
						$result['message'] = __( 'Your previous payment attempt has timed out. You can try again.', 'fair-audience-experimental' );
					} else {
						$result['payment_processing'] = true;
					}
				} elseif ( $transaction ) {
			...
			if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['pay_fee'] ) && 'pending' === $fee_payment->status ) {
				...
				$transaction_id = fair_payment_create_transaction(
					...
					array(
						...
						'description' => sprintf(
							...
							__( 'Membership fee: %s', 'fair-audience-experimental' ),
							$fee->name
						),
						'user_id'        => 0,
				...
				if ( is_wp_error( $transaction_id ) ) {
					$result['type']    = 'error';
					$result['message'] = __( 'Could not create payment. Please try again later.', 'fair-audience-experimental' );
				} else {
					...
					if ( is_wp_error( $payment_result ) ) {
						$result['type']    = 'error';
						$result['message'] = __( 'Could not initiate payment. Please try again later.', 'fair-audience-experimental' );
					} else {
...
<div class="fair-audience-fee-payment-container">
	<div class="fair-audience-fee-payment-box">
		<?php if ( ! $result['success'] ) : ?>
			<div class="fair-audience-error-container">
				...
				<h1 class="fair-audience-fee-payment-title">
					<?php echo esc_html__( 'Invalid Link', 'fair-audience-experimental' ); ?>
				</h1>
				...
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="fair-audience-fee-payment-link">
					<?php echo esc_html__( 'Return to Homepage', 'fair-audience-experimental' ); ?>
				</a>
			</div>
		<?php elseif ( 'paid' === $result['fee_payment']->status ) : ?>
			<div class="fair-audience-success-icon">&#10003;</div>
			<h1 class="fair-audience-fee-payment-title">
				<?php echo esc_html__( 'Payment Complete', 'fair-audience-experimental' ); ?>
			</h1>
			<p class="fair-audience-fee-payment-message success">
				<?php echo esc_html__( 'This fee has already been paid. Thank you!', 'fair-audience-experimental' ); ?>
			</p>

			<div class="fair-audience-fee-payment-details">
				<div class="fair-audience-fee-payment-detail-row">
					<span class="fair-audience-fee-payment-detail-label"><?php echo esc_html__( 'Fee:', 'fair-audience-experimental' ); ?></span>
					<span class="fair-audience-fee-payment-detail-value"><?php echo esc_html( $result['fee']->name ); ?></span>
				</div>
				<div class="fair-audience-fee-payment-detail-row">
					<span class="fair-audience-fee-payment-detail-label"><?php echo esc_html__( 'Amount:', 'fair-audience-experimental' ); ?></span>
				...
				<div class="fair-audience-fee-payment-detail-row">
					<span class="fair-audience-fee-payment-detail-label"><?php echo esc_html__( 'Paid:', 'fair-audience-experimental' ); ?></span>
		...
		<?php elseif ( 'canceled' === $result['fee_payment']->status ) : ?>
			<h1 class="fair-audience-fee-payment-title">
				<?php echo esc_html__( 'Payment Canceled', 'fair-audience-experimental' ); ?>
			</h1>
			<p class="fair-audience-fee-payment-message error">
				<?php echo esc_html__( 'This fee payment has been canceled.', 'fair-audience-experimental' ); ?>
			</p>
			<div class="fair-audience-fee-payment-footer">
				<a href="<?php echo esc_url( home_url( '/' ) ); ?>" class="fair-audience-fee-payment-link">
					<?php echo esc_html__( 'Return to Homepage', 'fair-audience-experimental' ); ?>
		...
		<?php elseif ( $result['payment_processing'] ) : ?>
			...
			<div class="fair-audience-fee-payment-processing" id="fair-audience-processing">
				...
				<p class="fair-audience-fee-payment-processing-text">
					<?php echo esc_html__( 'Your payment is being processed...', 'fair-audience-experimental' ); ?>
				</p>
			</div>
		...
		<?php else : ?>
			...
			<div class="fair-audience-fee-payment-details">
				...
				<div class="fair-audience-fee-payment-detail-row">
					<span class="fair-audience-fee-payment-detail-label"><?php echo esc_html__( 'Due Date:', 'fair-audience-experimental' ); ?></span>
			...
			<form method="post">
				...
				<button type="submit" class="fair-audience-fee-payment-submit">
					<?php echo esc_html__( 'Pay Now', 'fair-audience-experimental' ); ?>
				</button>
			</form>
		...
		if (attempt >= delays.length) {
			...
			if (el) {
				el.innerHTML = '<p class="fair-audience-fee-payment-processing-text">' +
					<?php echo wp_json_encode( __( 'Payment is taking longer than expected. Please reload this page to check the status.', 'fair-audience-experimental' ) ); ?> +
					'</p>';
			}
			return;
		}
