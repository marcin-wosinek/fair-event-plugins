<?php
/**
 * Instagram Connections REST API Controller
 *
 ...
 * Instagram Connections REST API controller
 */
class InstagramConnectionsController extends WP_REST_Controller {
	 ...
	 * @var string
	 */
	protected $rest_base = 'instagram-connections';

	/**
	 * Connection repository
	 *
	 * @var InstagramConnectionRepository
	 */
	private $repository;
	...
	public function __construct() {
		$this->repository = new InstagramConnectionRepository();
	}
	...
	public function register_routes() {
		// GET /fair-platform/v1/instagram-connections - List all connections.
		register_rest_route(
			$this->namespace,
			'/' . $this->rest_base,
			array(
		...
		// GET /fair-platform/v1/instagram-connections/stats - Get statistics.
		register_rest_route(
			$this->namespace,
		...
		// DELETE /fair-platform/v1/instagram-connections/cleanup - Cleanup old logs.
		register_rest_route(
			$this->namespace,
		...
		return new WP_REST_Response(
			array(
				...
				'message' => sprintf(
					...
					__( 'Deleted %d old Instagram connection logs.', 'fair-platform' ),
					$deleted
				),
			),
			200
	...
	public function get_items_permissions_check( $request ) {
		if ( ! current_user_can( 'manage_options' ) ) {
			return new WP_Error(
				'rest_forbidden',
				__( 'You do not have permission to view Instagram connection logs.', 'fair-platform' ),
				array( 'status' => 403 )
			);
		}
