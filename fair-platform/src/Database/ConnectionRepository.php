...
class ConnectionRepository {
	...
	public function __construct() {
		global $wpdb;
		$this->table_name = $wpdb->prefix . 'fair_platform_connections';
	}
	...
	public function log_connection( $data ) {
		...
		$result = $wpdb->insert(
			$this->table_name,
			$data,
			array( '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%s', '%s', '%s', '%s' )
		);
	...
	public function get_statistics() {
		...
		$stats = $wpdb->get_row(
			"SELECT
				...
				SUM(CASE WHEN profile_created = 1 THEN 1 ELSE 0 END) as profiles_created,
				COUNT(DISTINCT site_id) as unique_sites,
				COUNT(DISTINCT mollie_organization_id) as unique_organizations
