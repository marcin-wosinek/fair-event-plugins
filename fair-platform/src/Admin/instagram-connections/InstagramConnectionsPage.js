/**
 * Instagram Connections Admin Page Component
 */
...
/**
 * InstagramConnectionsPage component
 */
export default function InstagramConnectionsPage() {
	...
	const fetchConnections = async () => {
		...
		try {
			...
			const data = await apiFetch({
				path: `/fair-platform/v1/instagram-connections?${params.toString()}`,
			});
			...
			setError(
				err.message ||
					__('Failed to load Instagram connections', 'fair-platform')
			);
		} finally {
			setLoading(false);
		}
	...
	const fetchStatistics = async () => {
		try {
			const data = await apiFetch({
				path: '/fair-platform/v1/instagram-connections/stats',
			});
			setStatistics(data);
		} catch (err) {
	...
	const handleCleanup = async () => {
		if (
			!confirm(
				__(
					'Are you sure you want to delete old Instagram connection logs? This cannot be undone.',
					'fair-platform'
				)
			)
		) {
		...
		try {
			const data = await apiFetch({
				path: `/fair-platform/v1/instagram-connections/cleanup?days=${cleanupDays}`,
				method: 'DELETE',
			});
	...
	return (
		<VStack spacing={4}>
			<div>
				<h1>{__('Instagram Connection Logs', 'fair-platform')}</h1>
				<p>
					{__(
						'Monitor Instagram OAuth connections from WordPress sites.',
						'fair-platform'
					)}
				</p>
			</div>
					...
					<FlexBlock>
						<Card>
							<CardBody>
								...
								>
									{statistics.unique_usernames}
								</div>
								<div style={{ color: '#646970' }}>
									{__('Unique Usernames', 'fair-platform')}
								</div>
							</CardBody>
						</Card>
					</FlexBlock>
			...
			<Card>
				<CardHeader>
					<h2>
						{__('Instagram Connection History', 'fair-platform')}
					</h2>
				</CardHeader>
							...
							>
								<thead>
									<tr>
										...
										<th style={{ padding: '12px' }}>
											{__('IG Username', 'fair-platform')}
										</th>
										<th style={{ padding: '12px' }}>
											{__('IG User ID', 'fair-platform')}
										</th>
										<th style={{ padding: '12px' }}>
											{__(
												'Connected At',
