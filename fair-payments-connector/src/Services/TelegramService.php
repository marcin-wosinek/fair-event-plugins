<?php
/**
 * Telegram notification sender for Fair Payments Connector
 *
...
class TelegramService {
	...
	public function send( $bot_token, $chat_id, $text ) {
		...
		if ( '' === $bot_token ) {
			return new \WP_Error( 'fair_payment_telegram_missing_token', __( 'Telegram bot token is not configured.', 'fair-payments-connector' ) );
		}
		if ( '' === $chat_id ) {
			return new \WP_Error( 'fair_payment_telegram_missing_chat_id', __( 'Telegram chat ID is empty.', 'fair-payments-connector' ) );
		}
		...
		if ( is_wp_error( $response ) ) {
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				...
				error_log( '[Fair Payments Connector] Telegram send failed: ' . $response->get_error_message() );
			}
			return $response;
		}
		...
		if ( $code < 200 || $code >= 300 || empty( $data['ok'] ) ) {
			...
			if ( defined( 'WP_DEBUG' ) && WP_DEBUG ) {
				// phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
				error_log( '[Fair Payments Connector] Telegram send returned non-OK: ' . $description );
			}
			return new \WP_Error(
