...
use FairPaymentsConnector\Services\TelegramService;
	...
	public function send_test_message( $request ) {
		...
		if ( '' === trim( (string) $bot_token ) ) {
			return new \WP_Error(
				...
				__( 'Bot token is required.', 'fair-payments-connector' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( empty( $chat_ids ) ) {
			return new \WP_Error(
				...
				__( 'At least one chat ID is required.', 'fair-payments-connector' ),
				array( 'status' => 400 )
			);
		}
		...
		if ( ! $any_ok ) {
			return new \WP_Error(
				...
				__( 'Telegram test message failed for all chat IDs.', 'fair-payments-connector' ) . ' ' . implode( '; ', $errors ),
				array(
					'status'  => 502,
					'results' => $results,
				)
		...
		return new \WP_REST_Response(
			array(
				'success' => true,
				'message' => __( 'Telegram test message sent.', 'fair-payments-connector' ),
				'results' => $results,
				'text'    => $text,
			),
			200
