...
use FairEventsExperimental\Services\PostMigrationService;
...
class MigrationController extends WP_REST_Controller {
	...
	public function get_post_types( $request ) {
		...
		foreach ( $post_types as $post_type ) {
			// Exclude event post type.
			if ( Event::POST_TYPE === $post_type->name ) {
				continue;
			}
	...
	public function get_posts( $request ) {
		...
		if ( $post_type ) {
			...
			// Get all public post types except event.
			$post_types = get_post_types(
				array(
					'public' => true,
				),
			...
			unset( $post_types[ Event::POST_TYPE ] );
	...
	public function migrate_post( $request ) {
		...
		$event_id          = $migration_service->migrate_post( $post_id );
		...
		return rest_ensure_response(
			array(
				'success'  => true,
				'message'  => __( 'Post migrated successfully to event.', 'fair-events' ),
				'event_id' => $event_id,
			)
		);
	}
