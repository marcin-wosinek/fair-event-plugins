...
export default function MigrationApp() {
	...
	useEffect(() => {
		if (selectedPostType) {
			...
			setCategories([
				{ value: '', label: __('All Categories', 'fair-team') },
			]);
			setSelectedCategory('');
		}
	}, [selectedPostType]);
	...
	const loadPostTypes = async () => {
		try {
			const types = await apiFetch({
				path: '/fair-team/v1/migration/post-types',
			});

			setPostTypes([
				{ value: '', label: __('All Post Types', 'fair-team') },
				...types,
			]);
		} catch (error) {
			...
			setNotice({
				...
				message:
					error.message ||
					__('Failed to load post types.', 'fair-team'),
			});
		}
	};
	...
	const loadCategories = async (postType) => {
		try {
			const cats = await apiFetch({
				path: `/fair-team/v1/migration/categories?post_type=${postType}`,
			});
	...
	const loadPosts = async () => {
		...
		try {
			let path = '/fair-team/v1/migration/posts?per_page=20';
			if (selectedPostType) {
			...
			setNotice({
				status: 'error',
				message:
					error.message || __('Failed to load posts.', 'fair-team'),
			});
		} finally {
			setIsLoading(false);
		}
	...
	const handleMigrate = async (postId) => {
		...
		try {
			const response = await apiFetch({
				path: '/fair-team/v1/migration/migrate',
				method: 'POST',
				data: { post_id: postId },
			});
			...
			setNotice({
				...
				message:
					response.message ||
					__('Post migrated successfully!', 'fair-team'),
			});
			...
			setNotice({
				status: 'error',
				message:
					error.message || __('Failed to migrate post.', 'fair-team'),
			});
		} finally {
			setIsMigrating(null);
		}
	...
	return (
		<div className="wrap">
			<h1>{__('Migrate Posts to Team Members', 'fair-team')}</h1>
			...
			<div
				className="fair-team-migration-filters"
				style={{ marginBottom: '20px' }}
			>
				<div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
					<div style={{ minWidth: '200px' }}>
						<SelectControl
							label={__('Post Type', 'fair-team')}
						...
						{posts.map((post) => (
							<tr key={post.id}>
								...
								<td>
									...
									>
										{isMigrating === post.id
											? __('Migrating...', 'fair-team')
											: __('Migrate', 'fair-team')}
									</Button>
								</td>
							</tr>
						))}
