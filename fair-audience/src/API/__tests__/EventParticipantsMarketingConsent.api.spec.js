...
test.describe( 'EventParticipantsController marketing-consent', () => {
	...
	test( 'participant list exposes email_profile including declined', async () => {
		...
		const marketing = list.find(
			( p ) => p.participant_id === marketingId
		);
	...
	test( 'mixed request applies both directions in one call', async () => {
		...
		const list = await listRes.json();
		expect(
			list.find( ( p ) => p.participant_id === yesId ).email_profile
		).toBe( 'marketing' );
		expect(
			list.find( ( p ) => p.participant_id === noId ).email_profile
		).toBe( 'declined' );
