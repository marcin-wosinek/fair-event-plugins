  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      const rows = resultsData.participants.map((participant) => {
        ...
        resultsData.options.forEach((option) => {
          const isSelected = participant.selected_options.includes(option.id);
      ...
      // Save PDF
      const filename = `poll-results-${resultsData.poll.id}-${Date.now()}.pdf`;
      doc.save(filename);
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <div className="wrap">
      ...
      >
        ...
        <TextControl
          ...
          help={__("Internal reference for this poll.", "fair-audience")}
        />
        ...
        <TextareaControl
          ...
          help={__("The question shown to participants.", "fair-audience")}
          rows={3}
        />
