...
class QuestionnaireResponsesController extends WP_REST_Controller {
	...
	public function get_items( $request ) {
		...
		if ( ! empty( $participant_ids ) && class_exists( '\FairAudience\Database\ParticipantCategoryRepository' ) ) {
			$category_repo     = new \FairAudience\Database\ParticipantCategoryRepository();
			$categories_by_pid = $category_repo->get_categories_for_participants( $participant_ids );
		}
		...
		$participant_repo = class_exists( '\FairAudience\Database\ParticipantRepository' )
			? new \FairAudience\Database\ParticipantRepository()
			: null;

		$data = array();
		foreach ( $submissions as $submission ) {
			$participant = $participant_repo ? $participant_repo->get_by_id( $submission->participant_id ) : null;
	...
	public function get_item( $request ) {
		...
		if ( ! $submission ) {
			return new WP_Error(
				'not_found',
				__( 'Submission not found.', 'fair-form' ),
				array( 'status' => 404 )
			);
		}
		...
		if ( $request->has_param( 'event_date_id' ) ) {
			...
			if ( $event_date_id > 0 && class_exists( '\FairEvents\Models\EventDates' ) ) {
				...
				if ( ! $event_date ) {
					return new WP_Error(
						'invalid_event_date',
						__( 'Event date not found.', 'fair-form' ),
						array( 'status' => 400 )
					);
				}
			}
	...
	public function delete_item( $request ) {
		...
		if ( ! $submission ) {
			return new WP_Error(
				'not_found',
				__( 'Response not found.', 'fair-form' ),
				array( 'status' => 404 )
			);
		}
