...
test.describe( 'Events Calendar — Subscribe dropdown', () => {
	test( 'shows a single trigger that opens a dropdown with all subscription options', async ( {
		...
		await expect(
			page.locator( '.fair-events-subscribe-link' )
		).toHaveCount( 0 );

		const panel = page.locator( '.fair-events-subscribe-panel' );
		await expect( panel ).toBeHidden();
		await expect(
			panel.locator( '.fair-events-subscribe-note' )
		).toBeHidden();
		...
		expect( outlookHref ).toContain(
			'outlook.live.com/calendar/0/addfromweb'
		);
	...
	test( 'reflects the category filter in the feed URLs', async ( {
		page,
	} ) => {
		test.setTimeout( 60_000 );
