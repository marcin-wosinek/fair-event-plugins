 ...
 *
 * Output: assets/screenshot-1.png … screenshot-6.png (1200x900), or the
 * locale-suffixed set (e.g. screenshot-1-es.png) when SCREENSHOT_LOCALE is
 * set. The English and locale sets must be re-captured in the same run
 * (`npm run screenshots -w fair-events` then `npm run screenshots:es -w
 * fair-events`) so they never drift apart — see the plan in issue #1263.
 */

const VIEWPORT = { width: 1200, height: 900 };

const LOCALE = process.env.SCREENSHOT_LOCALE || '';
const ASSET_SUFFIX = { '': '', es_ES: '-es' }[ LOCALE ];
if ( ASSET_SUFFIX === undefined ) {
	throw new Error(
		`Unmapped SCREENSHOT_LOCALE "${ LOCALE }" — add it to ASSET_SUFFIX before capturing.`
	);
}

/**
 * Per-locale copy for seeded content, so a localized capture shows equivalent
 * sample data instead of English strings inside a translated UI. Keep dates
 * and times identical across locales so the two galleries read as the same
 * product.
 */
const COPY_BY_LOCALE = {
	'': {
		...
		buttonPageIntro:
			'Save the date — add this event to your calendar in one click.',
		seeds: [
			{
				title: 'Yoga Workshop',
				day: 5,
	...
	es_ES: {
		...
		buttonPageIntro:
			'Reserva la fecha — añade este evento a tu calendario con un clic.',
		seeds: [
			{
				...
				start_time: '10:00',
				end_time: '12:00',
				address: 'Pabellón del Parque, Avenida Amanecer 123',
			},
			{
				...
				start_time: '18:30',
				end_time: '21:00',
				address: 'Salón Comunitario, Calle Arce 45',
			},
			{
				...
				start_time: '20:00',
				end_time: '23:00',
				address: 'Café de la Esquina, Calle Sauce 7',
			},
			{
				...
				start_time: '09:00',
				end_time: '13:00',
				address: 'Aparcamiento del Sendero, Parque North Ridge',
			},
			{
				...
				start_time: '14:00',
				end_time: '16:30',
				address: 'Estudio de Artesanía, Bulevar Roble 88',
			},
		],
	},
};
const COPY = COPY_BY_LOCALE[ LOCALE ];
const ALL_PAGE_TITLES = Object.values( COPY_BY_LOCALE ).map(
	( c ) => c.pageTitle
);
...
test.describe( 'WordPress.org screenshots for Fair Events', () => {
	test( 'Generates admin + block screenshots from seeded data', async ( {
		...
		try {
			...
			// Switch the site language and, for a localized capture, load the
			// bundled translation catalog (the local Docker host has no WP.org
			// language pack for plugin strings). Always restored in `finally`
			// below so a Spanish run doesn't leave the dev box in Spanish.
			try {
				await apiFetch( page, {
					path: '/wp/v2/settings',
					method: 'POST',
					data: {
						language: LOCALE,
						fair_events_features: {
							'bundled-translations': !! LOCALE,
						},
					},
				} );
			} catch ( err ) {
				throw new Error(
					`Failed to set site language to "${ LOCALE }" — install the language pack first ` +
						`(wp language core install ${ LOCALE || 'en_US' }): ${
							err.message
						}`
				);
			}
			...
			// Drop any prior calendar pages from earlier runs, for every
			// locale — the English and localized captures share this site
			// within a single session, so a leftover page from the other
			// locale's run would otherwise leak into this run's nav menu.
			for ( const title of ALL_PAGE_TITLES ) {
			...
			const headlinePostId =
				headlinePost.event_id || headlinePost.post_id;
			...
			const buttonPostContent = [
				...
				'',
				'<!-- wp:paragraph -->',
				`<p>${ COPY.buttonPageIntro }</p>`,
				'<!-- /wp:paragraph -->',
				'',
				'<!-- wp:buttons -->',
				'<div class="wp-block-buttons"><!-- wp:button {"isCalendarButton":true} -->',
				`<div class="wp-block-button"><a class="wp-block-button__link wp-element-button">${ COPY.addToCalendarLabel }</a></div>`,
				'<!-- /wp:button --></div>',
				'<!-- /wp:buttons -->',
			].join( '\n' );
			await apiFetch( page, {
			...
			const now = new Date();
			const monthSlug = `${ now.getFullYear() }-${ pad(
				now.getMonth() + 1
			) }`;
			await page.goto(
			...
			await expect(
				page.getByText( headlineSeed.title ).first()
			).toBeVisible( {
				timeout: 15_000,
			} );
			...
			await page.screenshot( {
				path: `assets/screenshot-1${ ASSET_SUFFIX }.png`,
				fullPage: false,
			} );
			...
			await page.waitForTimeout( 800 );
			await page.screenshot( {
				path: `assets/screenshot-2${ ASSET_SUFFIX }.png`,
				fullPage: false,
			} );
			...
			await page.screenshot( {
				path: `assets/screenshot-3${ ASSET_SUFFIX }.png`,
				fullPage: false,
			} );
			...
			await page.waitForTimeout( 800 );
			await page.screenshot( {
				path: `assets/screenshot-4${ ASSET_SUFFIX }.png`,
				fullPage: false,
			} );
			...
			await page.waitForTimeout( 800 );
			await page.screenshot( {
				path: `assets/screenshot-5${ ASSET_SUFFIX }.png`,
				fullPage: false,
			} );
			...
			await page.waitForTimeout( 800 );
			await page.screenshot( {
				path: `assets/screenshot-6${ ASSET_SUFFIX }.png`,
				fullPage: false,
			} );
		} finally {
			// Restore the default site language/flag so a localized run
			// doesn't leave the dev box in Spanish for the next session. The
			// frontend screenshots above log the browser out, so re-login
			// first — otherwise this REST call is unauthenticated and fails
			// silently.
			await login( page );
			await page.goto(
				'/wp-admin/admin.php?page=fair-events-all-events'
			...
			await apiFetch( page, {
				...
				data: {
					language: '',
					fair_events_features: { 'bundled-translations': false },
				},
			} );
		}
	} );
