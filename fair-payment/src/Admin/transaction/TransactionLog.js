...
const TransactionLog = ({ transactionId }) => {
	...
	const load = () => {
		...
		setError(null);
		return apiFetch({
			path: `/fair-payment/v1/transactions/${transactionId}/log`,
		})
			.then((data) => {
			...
			.catch((err) => {
				setError(
					err.message ||
						__('Failed to load log entries.', 'fair-payment')
				);
			})
			.finally(() => {
				setReloading(false);
						...
						<thead>
							<tr>
								<th style={{ width: '160px' }}>
									{__('Time', 'fair-payment')}
								</th>
								<th style={{ width: '70px' }}>
									{__('Level', 'fair-payment')}
								</th>
								<th style={{ width: '180px' }}>
									{__('Event', 'fair-payment')}
								</th>
								<th>{__('Message', 'fair-payment')}</th>
								<th style={{ width: '110px' }}>
									{__('Request', 'fair-payment')}
								</th>
							</tr>
						</thead>
						<tbody>
