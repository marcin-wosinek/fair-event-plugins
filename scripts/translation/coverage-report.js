...
async function calculateCoverage(plugin, locale) {
	...
	try {
		...
		const total = parsed.translations.filter((t) => !t.isHeader).length;
		...
		return {
			...
			percentage:
				total > 0 ? ((translated / total) * 100).toFixed(1) : '0.0',
		};
	} catch (error) {
		console.error(`Warning: Failed to parse ${poFilePath}:`, error.message);
		return null;
	}
}
	...
	for (const plugin of plugins) {
		...
		for (const locale of config.locales) {
			...
			if (coverage) {
				...
				report.byLocale[locale].summary.totalStrings += coverage.total;
...
function generateMarkdown(report) {
	...
	for (const [pluginName, pluginData] of Object.entries(report.byPlugin)) {
		md += `### ${pluginName}\n\n`;
		md += '| Locale | Total | Translated | Untranslated | Coverage |\n';
		md += '|--------|-------|------------|--------------|----------|\n';

		for (const [locale, coverage] of Object.entries(pluginData.locales)) {
	...
	try {
		...
		if (options.markdown) {
			...
			if (options.output) {
				...
				console.error(`✅ Markdown report saved to ${options.output}`);
			} else {
				console.log(md);
			}
		} else {
