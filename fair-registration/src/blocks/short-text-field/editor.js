...
registerBlockType('fair-registration/short-text-field', {
	icon: <FontAwesomeIcon icon={faFont} />,
	edit: ({ attributes, setAttributes, context }) => {
		const { label, placeholder, required, fieldId, maxLength } = attributes;
		const { 'fair-registration/formId': formId } = context;

		const blockProps = useBlockProps({
			className: 'fair-registration-short-text-field-editor',
		});
